...
If the pixel is inside the 150px radius we set the pixel opacity to 255 and the color to orange, if it is outside the 150px radius we set the color to orange and the opacity to pow(255*(1- (radius)/(450)),0.85) which assure us that the opacity will reach the value 0 at the distance 450 from the center giving a nice glowing effect.
The normal vector to the billboard is the normalized vector that starts from the center of the solar system and end in the view point.
To calculate the angles we utilise the function atan2.
The x angle is the arctangent of the ratio between the y component of the eye position and the length of the projection of the eye on the x-y plane.
The y angle is the arctangent of the ratio between the x and z components.
...
As seen in class, the normal matrix is transpose(inverse(M)) where M is the modelview matrix.
...
We compute the day color by combining the usual phong model with the clouds texture using the lambertian model. We mix them depending the cloud value.
